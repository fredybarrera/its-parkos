...
<template>
  ...
  >
    <div
      v-if="estado !== 'idle'"
      class="fixed bottom-4 left-1/2 z-40 flex w-fit max-w-[calc(100vw-2rem)] -translate-x-1/2 items-center gap-3 rounded-full border border-slate-200 bg-white/95 px-4 py-2 shadow-lg backdrop-blur"
    >
      <span
        class="h-2 w-2 shrink-0 rounded-full animate-pulse"
        :class="estado === 'running' ? 'bg-emerald-500' : 'bg-amber-500'"
      />
      ...
      <button
        type="button"
        class="shrink-0 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-200"
      ...
      <button
        type="button"
        class="shrink-0 rounded-full bg-rose-50 px-2.5 py-1 text-xs font-medium text-rose-600 transition-colors hover:bg-rose-100"
        @click="detener"
      >
        Detener
      </button>
